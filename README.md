...
答题区自带「罗马字 → 假名」输入引擎（`src/utils/romaji.js`），**不需要切换到系统的日文输入法**，
也**不会弹出系统 IME 的候选 / 联想窗**：

- 电脑上答题区是**不可编辑的展示区**（`div[tabindex]`）+ `window` 键盘事件。系统输入法只对可编辑元素生效，
  所以打罗马字时输入法完全不介入，没有任何候选窗、也没有输入法状态栏干扰
- 直接打罗马字：`ka` → `か`、`shinbun` → `しんぶん`、`kannpai` → `かんぱい`
- `-` 是长音符：`kopi-` → `コピー`；片假名单词会自动用片假名输出，其余用平假名
- 汉字、中文标点、emoji、空格等**非假名字符会被直接丢弃**，答题区里只可能出现假名
- `Backspace` 退格、`Esc` 清空、`Enter` 提交
- 触屏设备仍使用真实 `<input>`（需要软键盘），此时用系统日文输入法直接打假名也照常接受

> 说明：网页没有任何 API 能禁用操作系统输入法或关闭它的候选窗，所以这里走的是"绕开输入法"的路线。
...
src/
  ...
  utils/romaji.js            内置「罗马字 → 假名」输入引擎(过滤非假名字符)
...
- **区分**平假名与片假名：`はんえい` ✅ ／ `ハンエイ` ❌（会提示该用哪种）
  （汉字词直接打罗马字时输入框输出平假名，外来语自动输出片假名，一般不会写错种类）
